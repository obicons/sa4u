...
map<string, int> get_units_of_functions(const pugi::xml_document &doc, map<string, int> &unitname_to_id, int &nextid) {
        nextid = 0;
        map<string, int> functions_to_units;
        for (const pugi::xml_node &structure: doc.child("MDM").child("StructList")) {
                ...
                for (const pugi::xml_node &field: structure){
                        if(field.type() == pugi::node_element && field.name() == "Field"s && field.attribute("Units")) {
                                ...
                                functions_to_units[lmcp_field_to_get_function_name(structure_name, field_name)]=unitname_to_id[unit_type];
                                functions_to_units[lmcp_field_to_set_function_name(structure_name, field_name)]=unitname_to_id[unit_type];
                        }
                }
        }
