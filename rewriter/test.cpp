/**
 * g++ test.cpp -o test -I /usr/lib/llvm-14/include/ -std=c++17 -L/usr/lib/llvm-14/lib -lLLVM -Wl,--start-group -lclangAST -lclangASTMatchers -lclangAnalysis -lclangBasic -lclangDriver -lclangEdit -lclangFrontend -lclangFrontendTool -lclangLex -lclangParse -lclangSema -lclangEdit -lclangRewrite -lclangRewriteFrontend -lclangStaticAnalyzerFrontend -lclangStaticAnalyzerCheckers -lclangStaticAnalyzerCore -lclangCrossTU -lclangIndex -lclangSerialization -lclangToolingCore -lclangTooling -lclangFormat -Wl,--end-group
 */
#include <iostream>
#include <fstream>
#include <map>
...
public:
  ...
  void add_to_ctx(const std::string &str) {
    ctx.push_back(str);
  }
...
static std::string get_member_ref_qualified(ContextTracker &tracker, MemberExpr *expr) {
  std::string result = expr->getMemberNameInfo().getName().getAsString();
  for (auto child: expr->children()) {
    if (isa<MemberExpr>(child)) {
      result = get_member_ref_qualified(tracker, cast<MemberExpr>(child)) + "::" + result;
...
static std::string get_instrumentation_call(const std::string &varname, const std::string &expr) {
  ...
  return "_instrument_noclash(" + std::to_string(varname_to_id[varname]) + ",(" + expr + ")," + std::to_string(instance_no++) + ")";
}

static void instrument_function(Rewriter &rewriter, ASTContext &ctx, ContextTracker &tracker, BinaryOperator *op) {
  Expr* lhs = op->getLHS();
  ...
  // TODO: doesn't work with nested assignments, e.g. x = y = z
  std::string rhs_text;
  raw_string_ostream rhs_stream(rhs_text);
  //op->getRHS()->printPretty(rhs_stream, nullptr, PrintingPolicy(ctx.getLangOpts()));
  //rhs_text = std::string(rewriter.getRewrittenText(SourceRange(op->getRHS()->getBeginLoc(), op->getRHS()->getEndLoc())));
  SourceLocation rhs_begin, rhs_end;
  if (op->getRHS()->getBeginLoc().isMacroID())
    rhs_begin = sm.getFileLoc(sm.getImmediateExpansionRange(op->getRHS()->getBeginLoc()).getBegin());
  else
    rhs_begin = op->getRHS()->getBeginLoc();
  if (op->getRHS()->getEndLoc().isMacroID())
    rhs_end = sm.getFileLoc(sm.getImmediateExpansionRange(op->getRHS()->getEndLoc()).getEnd());
  else
    rhs_end = op->getRHS()->getEndLoc();
  rhs_text = std::string(rewriter.getRewrittenText(SourceRange(rhs_begin, rhs_end)));
  ...
  stream << "=" << rhs_text;
  std::string instrumented_assignment = get_instrumentation_call(lhs_qual, stmt);

  //rewriter.RemoveText(op->getSourceRange(), options);
  //rewriter.InsertTextBefore(op->getBeginLoc(), instrumented_assignment);
  //rewriter.ReplaceText(op->getSourceRange(), instrumented_assignment);
  //rewriter.ReplaceText(SourceRange(sm.getSpellingLoc(op->getBeginLoc()), sm.getSpellingLoc(op->getEndLoc())), instrumented_assignment);
  //rewriter.RemoveText(SourceRange(op->getBeginLoc(), sm.getExpansionLoc(op->getEndLoc())));
  if (op->getEndLoc().isMacroID())
    rewriter.RemoveText(SourceRange(op->getBeginLoc(), sm.getFileLoc(sm.getImmediateExpansionRange(op->getEndLoc()).getEnd())));
  else
    rewriter.RemoveText(SourceRange(op->getBeginLoc(), op->getEndLoc()));

  rewriter.InsertText(sm.getSpellingLoc(op->getBeginLoc()), instrumented_assignment);
}

class RewritingVisitor : public RecursiveASTVisitor<RewritingVisitor> {
public:
  RewritingVisitor(Rewriter &R, ASTContext &c) : TheRewriter(R), ast_context(c), compound_stmt_depth(0) {}
  ...
  bool VisitBinaryOperator(BinaryOperator *op) {
    if (op && op->getOpcode() == BO_Assign) {
      Expr *lhs = op->getLHS();
      std::string filename(ast_context.getSourceManager().getFilename(op->getExprLoc()));
      if (lhs && isa<MemberExpr>(lhs) && !op->getLHS()->refersToBitField() && !op->isInstantiationDependent() && path_writable(filename)) {
...
public:
  MyASTConsumer(Rewriter &R, ASTContext &a, const std::string &file) : Visitor(R, a), file(file) {}
  ...
  bool HandleTopLevelDecl(DeclGroupRef DR) override {
    for (DeclGroupRef::iterator b = DR.begin(), e = DR.end(); b != e; ++b) {
      ...
      if (isa<CXXMethodDecl>(decl)) {
        ...
        if (d->getTemplateSpecializationInfo() || d->getTemplateInstantiationPattern())
          continue;
      }
...
public:
  ...
  std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI,
    ...
    //TheRewriter.InsertText(sm.getLocForStartOfFile(sm.getMainFileID()), instrumentation_function);
    return std::make_unique<MyASTConsumer>(TheRewriter, CI.getASTContext(), the_file);
  }
...
int main(int argc, const char **argv) {
  if (argc != 3) {
    std::cerr << "usage: " << argv[0] << " [compilation database path] [path to instrumentation source code]" << std::endl;
    return 1;
  }
  ...
  auto compilation_database = CompilationDatabase::loadFromDirectory(argv[1], msg);
  if (!compilation_database) {
	  llvm::errs() << "HERE cannot load compilation database\n";
	  return 1;
  }

  ClangTool Tool(*compilation_database, ArrayRef<std::string>(compilation_database->getAllFiles()));
